...
- 孙一斐：
- 刘耘希：
- 徐常喻：智能体在生成标准化代码方面效率很高，这是这次实践中最直观的体验。Model 层的十个卡牌子类需要各自覆盖 `canUse`/`canTarget`/`getValidTargets`/`execute` 四个虚接口，结构相同但逻辑不同——如果手写，每个子类要重复头文件声明、cpp 文件定义、构造函数链式调用等大量模板代码。智能体一次就能把这十个类全部生成到位，我只需要逐类检查编译和边界条件即可。同样，GameRule 中多个 AOE 响应函数（南蛮入侵和万箭齐发结构对称，区别只是响应牌型不同）也由智能体按照已有模式快速生成，省去了大量复制粘贴后再逐处修改的工作。此外，四个武将子类（曹操、关羽、张飞、赵云）的技能框架也由智能体按 `Character` 基类接口快速完成。可以不夸张地说，如果没有智能体辅助，Model 层的编码工作量至少要翻倍。
