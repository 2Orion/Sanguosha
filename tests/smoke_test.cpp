...
int main()
{
    ...
    check(caoCao.triggerCondition(GameEvent::OnDamage, &state, &p1), "曹操-奸雄: 受到伤害事件触发条件成立");
    check(!caoCao.triggerCondition(GameEvent::OnCardPlayed, &state, &p1), "曹操-奸雄: 无关事件不触发");

    p3.setUsedKillThisTurn(true);
    check(GameRule::canPlayKill(&state, &p3), "张飞-咆哮: 即使本回合已用杀仍可再次使用");
    p3.resetTurnState();

    // ==================== 4. Card 多态接口（Kill / Peach） ====================
    KillCard sampleKill(CardSuit::Spade, 7);
    check(sampleKill.canUse(&state, &p1), "KillCard: 出牌阶段、未用过杀时可使用");
    check(!sampleKill.canTarget(&state, &p1, &p1), "KillCard: 不能指向自己");
    check(sampleKill.canTarget(&state, &p1, &p2), "KillCard: 可指向其他存活玩家");
    check(sampleKill.getValidTargets(&state, &p1).size() == 2, "KillCard: 合法目标为除自己外的2人");

    ActionResult killResult = sampleKill.execute(&state, &p1, {&p2});
    ...
    check(p1.hasUsedKillThisTurn(), "KillCard::execute 标记本回合已用杀");
    ...
    check(samplePeach.canUse(&state, &p2), "PeachCard: 受伤后可使用");
    check(samplePeach.canTarget(&state, &p2, &p2), "PeachCard: 只能指向自己");
    check(!samplePeach.canTarget(&state, &p2, &p1), "PeachCard: 不能指向他人");
    ActionResult peachResult = samplePeach.execute(&state, &p2, {&p2});
    check(peachResult == ActionResult::Completed, "PeachCard::execute 返回 Completed");
    check(p2.hp() == 4, "接口测试: p2 回复至满血");

    // ==================== 5. GameRule: 杀/闪循环 + 酒 ====================
    Card* dodgeForP2 = takeOfType(pool, CardType::Dodge);
    check(dodgeForP2 != nullptr, "测试准备: 取到一张闪");
    p2.addHandCard(dodgeForP2);

    check(GameRule::canPlayKill(&state, &p1), "规则: p1 本回合尚未用杀，可以出杀");
    GameRule::executeKill(&state, &p1, &p2);
    check(p1.hasUsedKillThisTurn(), "杀: 攻击者标记本回合已用杀");
    check(state.pendingActionInfo().target == &p2, "杀: 待响应目标为p2");
    GameRule::handleKillResponse(&state, &p2, dodgeForP2);
    check(p2.hp() == 4, "闪: 成功闪避不掉血");
    check(!p2.hasCard(dodgeForP2), "闪: 闪牌从手牌移除");
    check(!state.hasPendingAction(), "闪: 响应后待定动作清空");
    p1.resetTurnState();

    GameRule::executeKill(&state, &p1, &p2);
    GameRule::handleKillResponse(&state, &p2, nullptr);
    check(p2.hp() == 3, "杀(未闪): 扣1点血");
    p1.resetTurnState();

    GameRule::executeWine(&state, &p1);
    check(p1.isWineEnhanced(), "酒: 攻击者标记酒强化");
    GameRule::executeKill(&state, &p1, &p2);
    GameRule::handleKillResponse(&state, &p2, nullptr);
    check(p2.hp() == 1, "酒+杀(未闪): 扣2点血");
    check(!p1.isWineEnhanced(), "酒: 使用后强化状态被消耗");
    p1.resetTurnState();

    // ==================== 6. GameRule: 桃 ====================
    Card* peachForP2 = takeOfType(pool, CardType::Peach);
    check(peachForP2 != nullptr, "测试准备: 取到一张桃");
    p2.addHandCard(peachForP2);
    GameRule::executePeach(&state, &p2, &p2);
    check(p2.hp() == 2, "桃: 回复1点体力");
    p2.removeHandCard(peachForP2);
    ...
    p2.addHandCard(redNonKill);
    check(GameRule::hasKillToRespond(&p2), "关羽-武圣: 红牌可当杀响应南蛮入侵");
    ...
    p2.addHandCard(lootA);
    GameRule::executeDismantle(&state, &p1, &p2);
    check(!p2.hasCard(lootA), "过河拆桥: 目标手牌被弃置");
    ...
    p3.addHandCard(lootB);
    GameRule::executeSteal(&state, &p1, &p3);
    check(!p3.hasCard(lootB), "顺手牵羊: 目标手牌被拿走");
    check(p1.hasCard(lootB), "顺手牵羊: 发动者获得该牌");
    ...
    // ==================== 11. 桃园结义 ====================
    int p1HpBefore = p1.hp();
    int p2HpBefore = p2.hp();
    int p3HpBefore = p3.hp();
    GameRule::executePeachGarden(&state);
    check(p1.hp() == std::min(p1HpBefore + 1, p1.maxHp()), "桃园结义: p1回复1点(未满则+1)");
    check(p2.hp() == std::min(p2HpBefore + 1, p2.maxHp()), "桃园结义: p2回复1点(未满则+1)");
    check(p3.hp() == p3HpBefore, "桃园结义: p3已满血不受影响");
    check(p1.isFullHp(), "桃园结义后: p1恢复满血");

    // ==================== 12. 南蛮入侵：多目标链式响应（回归测试） ====================
    Card* killForP2 = takeOfType(pool, CardType::Kill);
    check(killForP2 != nullptr, "测试准备: 给p2一张杀用于响应南蛮入侵");
    p2.addHandCard(killForP2);

    GameRule::executeBarbarianInvasion(&state, &p1);
    check(state.hasPendingAction() && state.pendingActionInfo().target == &p2,
          "南蛮入侵: 第一个响应目标为p2");
    check(state.pendingActionInfo().canSkip, "南蛮入侵: 可跳过标记为true");

    int p2HpBeforeAoe = p2.hp();
    GameRule::handleAoeKillResponse(&state, &p2, killForP2);
    check(p2.hp() == p2HpBeforeAoe, "南蛮入侵: p2出杀，不掉血");
    check(state.hasPendingAction() && state.pendingActionInfo().target == &p3,
          "南蛮入侵: 链式推进到p3");

    int p3HpBeforeAoe = p3.hp();
    GameRule::handleAoeKillResponse(&state, &p3, nullptr);
    check(p3.hp() == p3HpBeforeAoe - 1, "南蛮入侵: p3未出杀，扣1点血");
    check(!state.hasPendingAction(),
          "南蛮入侵(回归): 两名目标都已响应后链条正确终止，不会重复询问p2");

    // ==================== 13. 濒死流程：单人救援成功 ====================
    GameRule::dealDamage(&state, &p2, p2.hp() - 1, &p1);
    check(p2.hp() == 1, "濒死准备: p2降至1点血");

    GameRule::dealDamage(&state, &p2, 1, &p1);
    check(p2.hp() == 0, "濒死: 致命伤害后hp为0");
    check(p2.isDying(), "濒死: 玩家被标记为濒死");
    ...
    check(state.pendingActionInfo().target == &p2, "濒死: 待定动作目标为濒死玩家");
    ...
    // ==================== 15. 胜负判定 ====================
    GameRule::dealDamage(&state, &p3, p3.hp() - 1, &p1);
    GameRule::dealDamage(&state, &p3, 1, &p1);
    ...
    check(state.winner() == &p1, "终局: 获胜者为p1");
