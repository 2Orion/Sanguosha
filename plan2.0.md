...
当前测试覆盖 Model、ViewModel、View、App 和 Network；`NetworkTest -functions` 当前列出 63 个测试函数。
...
- **普通锦囊（26）**：过河拆桥×3、顺手牵羊×3、无中生有×3、南蛮入侵×3、万箭齐发×3、桃园结义×1、决斗×3、借刀杀人×2、五谷丰登×2、无懈可击×3
- **延时锦囊（6）**：乐不思蜀×3、兵粮寸断×2、闪电×1
- **装备牌（11）**：诸葛连弩×2、青龙偃月刀/丈八蛇矛/麒麟弓/青釭剑/寒冰剑/雌雄双股剑/仁王盾各×1、八卦阵×2
- **总计**：101 张

### 当前武将
曹操（奸雄）、关羽（武圣）、张飞（咆哮）、赵云（龙胆）、孙权（制衡）、周瑜（英姿）、吕布（无双）、大乔（流离）、司马懿（反馈）。

### 当前完成度边界

- 决斗、基本牌、6 种旧锦囊、AOE/濒死链、延时锦囊判定和诸葛连弩出杀次数限制已接入主结算。
- 无懈可击已接入过河拆桥、顺手牵羊、决斗、借刀杀人、乐不思蜀和兵粮寸断的单层响应；反无懈连锁尚未实现。借刀杀人和五谷丰登仍为简化结算。
- 周瑜英姿、孙权制衡（选择手牌且每回合限一次）和吕布无双（连续两张闪）已接入本地/网络流程；大乔流离仍只有 Model 查询接口。
