...
> ⚠️ 原稿的 4.1（运行一段时间后无响应退出）和 4.3（有可响应牌时必须响应不可跳过）中的部分问题已经在实际代码里修复，且根因和原稿的推测不一样。原稿是问题发现前基于猜测写的排查清单，真正的根因、修复方式和涉及文件见 [`CHANGELOG.md`](CHANGELOG.md)。当前仍需为 ViewModel 的响应校验、濒死链和网络非法命令补回归测试。
>
