...
当前测试覆盖 Model、ViewModel、View、App 和 Network；`NetworkTest -functions` 当前列出 59 个测试函数。
...
- 决斗、基本牌、6 种旧锦囊、AOE/濒死链和诸葛连弩出杀次数限制已接入主结算。
- 借刀杀人和五谷丰登为简化结算；闪电、无懈可击、乐不思蜀、兵粮寸断的完整规则未实现。
- 周瑜英姿已接入摸牌阶段；孙权制衡、吕布无双、大乔流离只有 Model 查询接口，尚未接入操作/结算流程。
...
- `PlayerData`（`src/Common/PlayerData.h`）实际落地为 `QVector<CardData> equipCards`，只包含已装备的非空槽，并附带 `attackRange`。`GameViewModel::pushPlayerData` 负责赋值。
...
1. **甲先提交 Common 新字段的结构定义**——✅ 已完成：`PlayerData::equipCards`/`attackRange`、`CardData::isEquipment`/`equipSlot`/`attackRange`、`CommonTypes.h` 的 `EquipSlot` 枚举与 9 种装备 `CardType`。坐骑（Mount）槽位已预留枚举、暂无对应牌。乙的序列化以这些实际字段为准。
