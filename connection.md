...
| `judgmentPerformed(CardData, QString, bool)` | `onJudgmentPerformed` | 判定牌、结果文本、生效标记 |
...
| `skillRequested(QVector<int>, int)` | `ActionViewModel::onSkillRequested` |
...
void SGSApp::startLocalGame(int charId1, int charId2) {
    ...
    connect(m_board, &GameBoardWidget::skillRequested,       avm,   &ActionViewModel::onSkillRequested);
    ...
    connect(m_gvm, &GameViewModel::judgmentPerformed,    m_board, &GameBoardWidget::onJudgmentPerformed);
...
struct PlayerData {
    ...
    QVector<CardData> judgmentCards;
};
...
| `judgmentPerformed(CardData, QString, bool)` | `JudgmentPerformed` | `JudgmentPerformedMsg` |
...
| `SkillRequested` | `SkillRequestedMsg` | `ActionViewModel::onSkillRequested` |
...
`DiscardCardMsg::playerId`、`SkillRequestedMsg::playerId` 都是客户端消息体里自己填的字段，**服务器分发时一律忽略它们，
...
**`AdvanceRequested` 阶段门控（2026-07-17）**：`ServerApp` 在 **Discard** 阶段拒绝 Advance（弃牌须走
确认弃牌命令，不能靠 Advance 跳过）。Prepare/Judge/Draw/End/Play 在通过当前玩家校验后仍转发；
其中 Play 作为 `endPlayPhase` 的等价路径保留（`GameViewModel::advancePhase` 在 `hasPendingAction`
时拒绝离开 Play）。另：`GameBoardWidget::onPhaseChanged` 换阶段时必须先 `stop` 再按需 `start`
`m_autoAdvanceTimer`，否则 Draw 的 300ms 定时器会在进入 Play 后仍触发，把 Play 误推进到 Discard
（网络 e2e「点杀无响应」的主因之一）。`GameViewModel::advancePhase` 在 `m_judgePending` 时亦拒绝
外部推进，避免与判定展示定时器竞态。
...
| `useSkill(const QVector<int>& cardIds, int playerId)` | `SkillRequested` | `SkillRequestedMsg` |
...
（8 个对局命令发送方法逐一验证 payload 字段）、`gameClientReceivesGameStartedAndBroadcastsWithRedaction`
...
**网络模式仍复用同一个 `GameBoardWidget`，未引入任何 Model/ViewModel 依赖。** `GameBoardWidget` 从设计
上不知道对面接的是本地 `GameViewModel`/`ActionViewModel` 还是网络 `GameClient`，因为
...
| View → Client | `skillRequested` | `GameClient::useSkill` |
...
| Client → View | `judgmentPerformed` | `GameBoardWidget::onJudgmentPerformed` |
...
不会被误踢。当时阶段性统计为 60 个用例；当前 `NetworkTest -functions` 列出 63 个测试函数。
...
当时阶段性统计为 61 个用例；当前 `NetworkTest -functions` 列出 63 个测试函数。
