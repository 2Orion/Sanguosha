...
> **文档语义**：第 0 节是当前代码状态的权威摘要；后续章节同时保留设计方案和实施历史。
> 标注“已落地/已完成”的内容应与当前代码一致；未标注的代码块或效果表仍可能是待实现方案，不代表当前功能已完整接入。

---

## 0. 当前状态

> 本节按当前代码的 MVVM 结构更新：View 层零 ViewModel 依赖；本地模式由
> `SGSApp::startLocalGame()` 直连 View 与 ViewModel，网络模式由 `ServerApp` 和 `ClientApp` 组装。
...
Network 层                    ✅ 完整 — Protocol、MessageSerializer、GameServer、GameClient
App 层 (组合根)              ✅ 完整 — SGSApp（本地/联网入口）、ServerApp、ClientApp
自动化测试                  ✅ 5/5 目标（ModelSmokeTest、ModelTest、ViewModelTest、ViewTest、NetworkTest）
```

当前测试覆盖 Model、ViewModel、View、App 和 Network；`NetworkTest -functions` 约 65 个测试函数。
...
### 当前卡牌
- **基本牌（58）**：杀×30、闪×15、桃×8、酒×5
- **普通锦囊（24 入堆）**：过河拆桥×3、顺手牵羊×3、无中生有×3、南蛮入侵×3、万箭齐发×3、桃园结义×1、决斗×3、借刀杀人×2、无懈可击×3；五谷丰登×2 **类保留、暂不入堆**
- **延时锦囊（6）**：乐不思蜀×3、兵粮寸断×2、闪电×1
- **装备牌（7 入堆）**：诸葛连弩×2、青龙偃月刀/青釭剑/寒冰剑/雌雄双股剑/仁王盾各×1；丈八蛇矛/麒麟弓/八卦阵×2 **类保留、暂不入堆**
- **总计入堆**：**95** 张（原 101 − 6）

### 当前武将
曹操（奸雄）、关羽（武圣）、张飞（咆哮）、赵云（龙胆）、孙权（制衡）、周瑜（英姿）、吕布（无双）、大乔（流离，**选将页 hidden**）、司马懿（反馈）。

### 当前完成度边界

- 决斗、基本牌、6 种旧锦囊、AOE/濒死链、延时锦囊判定和诸葛连弩出杀次数限制已接入主结算。
- 无懈可击已接入过河拆桥、顺手牵羊、决斗、借刀杀人、乐不思蜀和兵粮寸断的单层响应；反无懈连锁尚未实现。借刀杀人为两人局简化结算（目标须出杀否则交武器），响应路由已修正；五谷丰登不入堆。
- 周瑜英姿、孙权制衡（选择手牌且每回合限一次）和吕布无双（连续两张闪）已接入本地/网络流程。
- 关羽武圣：出牌/响应双路径转化（红牌当杀）；**不是**主动技能按钮。
- 大乔流离：`executeKill` 内有转移分支，两人局 `getRedirectTarget` 恒空；选将暂时隐藏。
- 防具/武器：仁王盾挡黑杀、青釭剑无视防具已在 `executeKill` 生效；距离加成已计入攻击范围。
- View：结算 log 改队列逐条顺序播放（每条约 1.2s），播完回退阶段文案（响应期间回退到常驻响应提示）；`ActionViewModel::logMessage`（出牌/响应/受伤/弃牌）已转发到 `GameViewModel::logMessage`，此前未转发导致出杀无闪掉血却无提示。
...
public:
    ...
    virtual bool ignoreArmor() const;         // 是否无视防具
...
> **已落地**：`Player` 内部使用 `m_equipSlots[4]` 按 `EquipSlot` 存储，
> `equipCard`/`unequipSlot`/`equippedAt`/`equipmentChanged` 是主接口；
> `equipCards`/`addEquipCard`/`removeEquipCard` 仅作为兼容接口保留。
...
| 牌名 | 花色点数 | 效果 |
|------|---------|------|
| 决斗 | ♠A, ♦A, ♣A | 目标先打出杀，然后双方轮流打出杀；未打出者受到对方造成的 1 点伤害 |
...
```cpp
namespace GameRule {
    void executeDuel(GameState* state, Player* user,
                     Player* target, Card* duelCard);
    void handleDuelResponse(GameState* state, Player* responder,
                            Card* killCard);
...
| 武将 | 体力 | 技能 | 类型 |
|------|------|------|------|
| 孙权 | 4 | 制衡：出牌阶段限一次，可弃任意张手牌并摸等量的牌 | 主动技能 |
...
当前 `Protocol::kProtocolVersion == 2`；v2 增加 `PlayerData::canUseActiveSkill`、`judgmentCards`、技能命令和判定结果广播。
...
enum class MessageType : uint8_t {
    ...
    SkillRequested,           // ~ ActionViewModel::onSkillRequested(cardIds, playerId)
    ...
    TargetSelectionStarted,    // ~ ActionViewModel::targetSelectionStarted(QVector<int>)
    TargetSelectionFinished,   // ~ ActionViewModel::targetSelectionFinished()
    JudgmentPerformed,         // ~ GameViewModel::judgmentPerformed(...)
...
│   │        targetSelectionStarted / targetSelectionFinished / judgmentPerformed
│   │        —— 内容由收到的网络消息反序列化后 emit，不经过任何本地 Model 计算
│   └── 方法：playCard / respondCard / selectTarget / discardCard / useSkill /
│              skipResponse / endPlayPhase / advancePhase
...
signals:
    ...
    void judgmentPerformed(const CardData& judgeCard,
...
> **实现说明**（`src/Network/GameClient.h/cpp`）：
...
网络模式不需要为 View 引入 Model/ViewModel 依赖或另一套控制器。`GameBoardWidget`、`PlayerInfoWidget`、`HandCardAreaWidget` 和 `ActionPanelWidget` 后续已因判定区、技能多选等通用功能扩展，但这些改动同时服务本地与网络模式；运行模式差异仍只由 `SGSApp`/`ClientApp` 的连接对象决定。

网络接入本身新增/修改的主要文件如下；判定区与技能多选属于共享 UI 功能，另行落在现有 View 文件中：
...
> **Step 9（进程内端到端对局 + 文档收尾）已完成，网络层 Step 1-9 全部落地**：单进程内 `ServerApp` +
> 2 个真实 `GameClient` 跑通握手→选将→出杀→响应→伤害→濒死救援→游戏结束的完整链路（M1-M3 预演），
> 以及游戏结束后的两条非法命令路径（M4 部分覆盖）。过程中发现并修正了测试自身手牌构造的非确定性
> 问题（Draw 阶段摸牌可能重新引入桃/酒、随机发牌可能让响应方没有闪，两者都会触达
> `GameViewModel` 的自动跳过分支导致预期的网络广播不发生）——这提醒了一个通用点：网络层测试凡是
> 依赖"待定动作一定会广播"的断言，都必须显式构造手牌排除自动跳过的可能性，不能依赖随机发牌。
> 详见 [`connection.md`](connection.md) §7.8。真机双开手动联调（§7.5 M1-M4）留给三人联调周。
...
> 当前回归测试入口为 `ctest --test-dir build --output-on-failure`，共 5 个目标：
> `ModelSmokeTest`、`ModelTest`、`ViewModelTest`、`ViewTest`、`NetworkTest`。
>
...
| 分层自动化测试（Model/ViewModel/View/App/Network） | ✅ 已完成（5 个 CTest 目标；无预期失败） |
...
1. **甲先提交 Common 新字段的结构定义**——✅ 已完成：`PlayerData::equipCards`/`judgmentCards`/`attackRange`/`canUseActiveSkill`、`CardData::isEquipment`/`equipSlot`/`attackRange`、`CommonTypes.h` 的 `EquipSlot` 枚举与 9 种装备 `CardType`。坐骑（Mount）槽位已预留枚举、暂无对应牌。
2. **乙随后冻结 `Network/Protocol.h`**——✅ 当前协议为 v2，消息枚举/结构体与 slots/signals 对齐，Common 值类型由 `QDataStream <<`/`>>` 序列化；字段变更必须继续递增版本号。
...
4. **View 层不得出现网络专属规则分支**：共享功能可以修改 `GameBoardWidget`，但本地/网络必须继续使用同一组值类型信号槽；如果只为网络模式引入 Model/ViewModel 依赖，应先回头检查 `ClientApp` 的连接设计。
